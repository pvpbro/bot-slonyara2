...
# Константы для фото
PHOTO_NORMAL_PATH = "/storage/emulated/0/фотобот/окак.jpg"
PHOTO_EXTINGUISHER_PATH = "/storage/emulated/0/фотобот/чертик.jpg"
PHOTO_TAXI_PATH = "/storage/emulated/0/фотобот/такси.jpg"
PHOTO_ACCOUNTANT_PATH = "/storage/emulated/0/фотобот/бухгалтер.jpg"
PHOTO_GANG_PATH = "/storage/emulated/0/фотобот/банды.jpg"
PHOTO_CASINO_PATH = "/storage/emulated/0/фотобот/казино.jpg"

# ==================== НОВЫЕ КОНСТАНТЫ ДЛЯ КОМБИНАЦИЙ ====================
PHOTO_FLOWER_WITH_EXTINGUISHER = "/storage/emulated/0/фотобот/цветок_с_огнетушителем.jpg"
PHOTO_FLOWER_WITHOUT_EXTINGUISHER = "/storage/emulated/0/фотобот/чертик.jpg"
...
def load_data():
        ...
        if os.path.exists('users.json'):
            with open('users.json', 'r', encoding='utf-8') as f:
                ...
                if content:
                    data = json.loads(content)
                    user_data = data.get('user_data', {})
                    banned_users = set(data.get('banned_users', []))  # Преобразуем в set
                    trade_offers = data.get('trade_offers', {})
                    active_trades = data.get('active_trades', {})
                else:
                    user_data = {}
    ...
    return {'user_data': user_data, 'banned_users': list(banned_users), 
            'trade_offers': trade_offers, 'active_trades': active_trades}
...
def save_data(data=None):
    ...
    if data is None:
        data = {
            'user_data': user_data, 
            'banned_users': list(banned_users),  # Важно!
            'trade_offers': trade_offers,
            'active_trades': active_trades
        }
    with open('users.json', 'w', encoding='utf-8') as f:
