...
async def business_collect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сбор дохода с бизнесов - ЗАЩИЩЕННАЯ ВЕРСИЯ"""
    query = update.callback_query
    await query.answer()
    ...
    if 'collect_count' not in user:
        user['collect_count'] = {'count': 0, 'reset_time': current_time + 3600}
    
    # Сбрасываем счетчик каждый час
    if current_time > user['collect_count']['reset_time']:
        user['collect_count'] = {'count': 0, 'reset_time': current_time + 3600}
    
    # Лимит 60 сборов в час (раз в минуту)
    if user['collect_count']['count'] >= 60:
        await query.answer("⏳ Слишком частые сборы! Подождите немного.", show_alert=True)
    ...
    # Минимальный интервал сбора - 60 секунд
    if seconds < 60:
    ...
    # 2. Доход с VIP бизнеса (30 млн в час) - ТОЛЬКО ЕСЛИ ЕСТЬ VIP СТАТУС!
    vip_hourly = 0
    if user.get('vip_business', False) and check_vip(user_id):
        vip_hourly = 30_000_000
        print(f"✅ VIP бизнес активен: +30,000,000 ₽/ч")
    ...
    # 5. Применяем VIP бонус 15% (только если есть VIP статус)
    ...
    # Увеличиваем счетчик сборов
    user['collect_count']['count'] += 1
    
    save_data()
    
    # ===== ФОРМИРУЕМ ДЕТАЛЬНЫЙ ОТЧЕТ =====
    normal_part = int((normal_hourly / 3600) * seconds)
    vip_part = int((vip_hourly / 3600) * seconds)
    bonus_part = earned - normal_part - vip_part
    
    minutes = int(seconds // 60)
    seconds_remain = int(seconds % 60)
    
    report = f"✅ +{earned:,} ₽ за {minutes} мин {seconds_remain} сек\n\n"
    report += f"📊 Обычные бизнесы: +{normal_part:,} ₽\n"
    
    if vip_part > 0:
        report += f"👑 VIP бизнес: +{vip_part:,} ₽\n"
    
    if bonus_part > 0:
        report += f"✨ VIP бонус 15%: +{bonus_part:,} ₽"
    
    await query.answer(report, show_alert=True)
