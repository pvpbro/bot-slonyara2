...
async def clicker_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем, правильная ли кнопка
    if button_idx == game['correct']:
        # ПОБЕДА! Начисляем награду
        reward = int(CLICKER_CONFIG['reward'] * level_mult * (1 + game['streak'] * 0.1))
        game['score'] += reward
        game['streak'] += 1
        game['total_clicks'] += 1
        ...
        # Эффект скорости (кнопка меняется быстрее на высоких уровнях)
...
async def business_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== ПЕРЕДАЧА КНОПОК ПОДТВЕРЖДЕНИЯ ПЕРЕВОДА (САМОЕ ВАЖНОЕ) =====
